...
/**
 * OneWayPermanentTrade represents a one-way permanent trade. Once the items have been given to the recipient, they will
 * not be returned. As such, these trades require only one meeting.
 *
 * @author Warren Zhu
 * @version %I%, %G%
 * @since Phase 1
...
public class OneWayPermanentTrade extends OneWayTrade implements Serializable {
    ...
    /**
     * Returns a List with a length equal to that of the number of traders involved in the trade. At each index, store
     * the ID of the item that is involved in the trade and will be own by the Trader at the given index in getTraders()
     * at the end of the trade. Iff the specified Trader will not receive an item that at the end of the trade, store
     * null at that particular index.
     *
     * @return A list of item IDs involved in this trade based on the original owners
     */
    public List<Integer> getItemsFinal(){
        return new ArrayList<>(Arrays.asList(null, getItem()));
    }
}